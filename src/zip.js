 ...
 *
 * Written by hand on purpose: the report only bundles CSV text, so there is
 * little to gain from deflate - and this keeps the center free of an extra
 * npm dependency on machines that sit behind the factory proxy.
 *
...
class ZipWriter {
  ...
  _write(buf) {
    ...
    if (this.offset > MAX_TOTAL) {
      this.close();
      throw new Error('export exceeds 4 GB - narrow the time range or meter selection');
    }
  }
